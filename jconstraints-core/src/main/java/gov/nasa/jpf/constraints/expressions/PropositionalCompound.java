...
public class PropositionalCompound extends AbstractBoolExpression {
  ...
  public PropositionalCompound(
      Expression<Boolean> left, LogicalOperator operator, Expression<Boolean> right) {
    this.operator = operator;
    this.left = left;
    this.right = right;
  }

  public static PropositionalCompound create(
      Expression<Boolean> left, LogicalOperator operator, Expression<?> right) {
  ...
  public Boolean evaluateSMT(Valuation values) {
    ...
    return rvUndecided || lvUndecided ? true : operator.eval(lv, rv);
  }
  ...
  public Expression<Boolean> duplicate(Expression<?>[] newChildren) {
    ...
    return new PropositionalCompound(
        newChildren[0].as(BuiltinTypes.BOOL), operator, newChildren[1].as(BuiltinTypes.BOOL));
  }
