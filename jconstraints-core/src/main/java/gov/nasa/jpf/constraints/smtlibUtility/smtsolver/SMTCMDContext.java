...
public class SMTCMDContext extends SolverContext implements StoppableSolver {
  ...
  private List<Expression> lastUnsatCore;
        ...
        if (l.startsWith("<stdin>")) {
          ...
          if (l.equals("sat")) {
            ...
            if (result != null) {
              ...
              SMTCMDSolver.getModel(ctx, bos, lvars, result);
            }
  ...
  private void analyze_core(String core) {
  ...
  public void add(List<Expression<Boolean>> expressions) {
    ...
    for (Expression f : expressions) {
      hasCalledAdd = true;
      String name = visitor.transform(f);
      if (unsatCoreSolver && smtExportConfig.namedAssert) {
        namedExpressions.put(name, f);
      }
    }
  ...
  protected List<Expression> getUnsatCore() {
