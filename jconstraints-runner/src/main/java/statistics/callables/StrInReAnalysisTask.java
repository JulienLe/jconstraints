...
public class StrInReAnalysisTask implements Callable<BooleanAnswer> {
  ...
  public boolean computeAnswer(Expression p) {
    ...
    if (p instanceof RegExBooleanExpression) {
      ...
      if ((castedP.getLeft() instanceof Variable || castedP.getLeft() instanceof Constant)
        ...
        if (roe.getOperator().equals(RegExOperator.ALLCHAR)) {
          return true;
        }
      }
    }
  ...
  public BooleanAnswer call() throws Exception {
    ...
    try {
      ...
      return new BooleanAnswer(
          file,
          problem.assertions.stream().map(p -> computeAnswer(p)).reduce(false, (a, b) -> a || b));
